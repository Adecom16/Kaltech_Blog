...
// Email Registration
export const emailRegister = asyncHandler(async (req, res, next) => {
  const { email, password, name } = req.body;

  if (!email || !password || !name) {
    throw new ServerError(400, "Email, password, and name are required");
  }

  // Check if user already exists
  const existingUser = await User.findOne({ email });
  if (existingUser) {
    throw new ServerError(400, "User with this email already exists");
  }
  ...
  // Generate tokens
  const access_token = jwt.sign({ _id: user._id }, env.JWT_SECRET, {
    expiresIn: "30m",
  });
  const refresh_token = jwt.sign({ _id: user._id }, env.JWT_REFRESH_SECRET);
...
export const emailLogin = asyncHandler(async (req, res, next) => {
  ...
  if (!email || !password) {
    throw new ServerError(400, "Email and password are required");
  }
